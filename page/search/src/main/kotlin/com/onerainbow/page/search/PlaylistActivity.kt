...
class PlaylistActivity : BaseActivity<ActivityPlaylistBinding>() {
	...
	fun Artist.toArtist(): com.onerainbow.page.search.data.Artist {
		return com.onerainbow.page.search.data.Artist(
			id = this.id,
			name = this.name,
			img1v1Url = " "
		)
	...
	fun com.onerainbow.page.search.data.Artist.toModelArtist(): Artist {
		return Artist(
			id = this.id,
			name = this.name
		)
