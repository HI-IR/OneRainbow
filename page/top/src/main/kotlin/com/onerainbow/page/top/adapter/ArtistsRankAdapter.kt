...
import com.onerainbow.page.top.R
import com.onerainbow.page.top.databinding.ItemArtistsBinding
...
class ArtistsRankAdapter(private val context: Context) :
    androidx.recyclerview.widget.ListAdapter<Artist, ArtistsRankAdapter.ViewHolder>(object : DiffUtil.ItemCallback<Artist>() {
        ...
        @SuppressLint("SetTextI18n")
            ...
            if (index in 0..2) {
                rank.apply {
                    //前3名用特殊颜色+字体加大
                    text = "${index + 1}"
                    setTextColor("#17182C".toColorInt())
                    textSize = 18f
                }
            } else {
                rank.apply {
                    text = "${index + 1}"
                    setTextColor("#868894".toColorInt())
                    textSize = 14f
                }
            }
        }
